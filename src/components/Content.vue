<template>
  <div class="flex flex-wrap">
    <div class="w-full lg:w-9/12 bg-gray-800 py-6 px-6 rounded-3xl">
      <!--div class="flex justify-between text-white items-center mb-8">
        <p class="text-2xl font-bold">{{ selectImage.name }}</p>
      </!div-->
      <div class="flex flex-wrap">
        <el-image style="width: 100%; height: 71%" :src="selectImage.url" :preview-src-list="previewlist"
          :initial-index="4" fit="contain" />
      </div>
    </div>
    <div class="w-full mt-8 lg:mt-0 lg:w-3/12 lg:pl-4">
      ...
      <el-table v-show="images && images.length > 0" style="width: 100%" max-height="100vh" :data="images" fit>
        ...
        <el-table-column align="center">
          <template #default="{ row }">
            <el-image :src="row.url" fit="contain" style="height: 210px; width:300px;" @click="showImage(row)" />
          </template>
        </el-table-column>
      </el-table>
    </div>
